...
with open("House.pkl",'rb')as f:
    model=pickle.load(f)
#create a function to accept inputs and create an aaray
...
if __name__=="__main__":
    ...
    Type=col1.selectbox("Select the Type",list(property_type_mapping.keys()))
    result=predict(bed,bath,loc,size,status,facing,Type)
    submit_button=st.button("Submit")
    if submit_button:
        larger_text=f"<h2 style='color:light pink:'>The Predicted House is:{result} Lakhs</h2>"
